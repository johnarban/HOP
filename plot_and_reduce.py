...
def plot_reduce(cal_file, data_file, cal_threshold=0.05, bottom = None, top = None, rectify=True, order=2,
           plot=True,recal='n',cosmic_rays=False,save=False, slice_fwhm = 1.5,size=5):
    ...
    with plt.rc_context(rc={'image.origin': 'upper'}):
        ...
        sl = hf.specextract(data,bottom=bottom,top=top,plot=plot,ax=axs['I'],slice_fwhm=slice_fwhm)
...
if __name__ == "__main__":
    ...
    out,fig = plot_reduce(cal_file, data_file, cal_threshold=threshold, bottom = bottom, top = top, order=order, rectify=rectify, cosmic_rays=zap_cosmic_rays, plot=plot, save=save, slice_fwhm=slice_fwhm, recal=recal)
