...
def wavelength_cal(cal_file,recal='n',threshold=0.05,size=5, order=2, rect_order=2):
    ...
    if (exists(fname) & (recal=='y')) | (not exists(fname)):
        ...
        while make_selection:
            ...
            key = input('And then press any key to continue [q to quit]:')
        ...
        wavesol = np.polyfit(np.sort(cal_locs)[::-1],np.sort(line_locs),order)
